...
def main(hparams={}):
    ...
    config.train.checkpoint_dir = "ckpts/sft_arc_english_llama7B_croatianEval"
    # config.train.epochs = 100
    config.train.batch_size = 2
    # config.train.minibatch_size = 2
    config.train.project_name = "sft_arc_llama7B_croatian_croatianEval"
    config.train.run_name = "english_train"
    ...
    def metric_fn(samples: List[str], **kwargs):
        split_names = ["iid_test", "croatian_test"]
        output_dict = {}
    ...
    test_prompts = english_test_prompts
    ...
    # iid_test_prompts = list(map(prepare_prompt, test_questions, test_choices, test_answers, [0 for _ in range(len(test_questions))]))
    # test_prompts += iid_test_prompts




    np.random.shuffle(train_samples)
    np.random.shuffle(test_prompts)
    train_samples = train_samples[:1115]
    ...
    ood_test_prompts =  list(map(prepare_prompt, ood_questions, ood_choices, ood_answers, [1 for _ in range(len(ood_questions))]))
    ood_test_prompts = ood_test_prompts
